...
int main(int argc, char const *argv[])
{
    ...
    if(child > 0) {
        ...
        // After the process getting forked we go ahead and call execvp
        // execvp takes in two parameters 
        // first parameter is the process name
        // second parameter is the file descriptor so the re-exec child process has access 
        execvp(pwd->pw_name, server_fd);
